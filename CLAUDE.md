...
- **Source of truth**: **markdown files on the user's disk, tracked in a git repo** — **IMPLEMENTED** in `server/src/vault/` (Round 5). YAML frontmatter (structured fields) + freeform body (the description); Obsidian-usable without QueryKey. `VAULT_DIR` selects the root (default `./vault`); layout `people/ tasks/ events/ notes/ conflicts/ questions/ followups/`. API + ingest write the vault first, then project a **derived** index into Loca; the graph is rebuilt from the vault on startup; `update_task` / `resolve_conflict` / `resolve_question` / `create_followup` mutate the markdown. Round-trips are lossless (unit-tested). Spec + as-built notes: `docs/markdown-schema.md`. Person/Task/Event **and** Conflict/OpenQuestion/FollowUp (Round 6) **and** Instruction/VoiceProfile (Round 10) now have canonical on-disk forms — the **full entity set is on disk**, nothing graph-only. **Semantic wikilinks (Round 8):** any entity body may carry `[[Target]]` / `[[property:Target]]` (single-colon typed triples, not SMW `::`) — resolved (precedence + dangling) into derived edges; live at `/api/links` + per-entity backlinks, also projected into the triple store. Spec: `docs/markdown-schema.md` "Semantic wikilinks". **Status-workflow (Round 9):** Task/Conflict/Question transitions (`src/workflow`) enforced at the API mutation boundary (a resolved conflict can't un-resolve; `done` can't rewind to `extracted`) — *not* in the vault, so hand-edited markdown stays legal. **Calendar (Round 11):** optional Event `recurrence` (RFC-5545 subset, `src/calendar`) + `GET /api/calendar` merged agenda (event occurrences + deadlined tasks, movable-vs-fixed), live from the vault.
