...
- **Source of truth**: **markdown files on the user's disk, tracked in a git repo.** Working format: **YAML frontmatter** for structured fields (person, date, tags, status) + freeform markdown body (Obsidian convention — usable without QueryKey installed). Document/finalize the schema before writing ingestion code; do not implement the on-disk model until then.
- **Knowledge graph**: **derived from the markdown, not canonical.** RDF/graph is generated *out of* the files; it is a rebuildable secondary index. Store: **Loca** (formerly **SutraDB**) — the author's own embedded **Rust** graph-vector-time DB (separate project; the time dimension matters — you care about a relationship's history). **Fuseki is NOT used** — the Go `server/internal/graph/` Fuseki stub is stale pre-pivot scaffolding slated for removal; do not build on it or re-document Fuseki as the plan.
- **AI engine**: model-agnostic. QueryKey exposes an **MCP server** (present from day one) so *any* agent can attend over the graph and act on the files. Default agent: local **Gemma** (cheap, private); Claude/GPT optional for power users. *Implementation note:* today's bridge is OpenClaw via a WSL gateway (`127.0.0.1:18789`) under `server/internal/openclaw/` — an implementation detail to be superseded by the Rust rewrite. Callers must never name a specific model/engine.
- **Server**: local-first by design — ingestion, the derived graph, MCP, real-time sync. **Target language: Rust.** The current `server/` is Go and is **deprecated** (kept compilable until the Rust rewrite supersedes it; no rewrite this round).
