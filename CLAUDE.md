...
- **Server**: **Rust** (`server/`, crate `querykey-server`) — local-first: ingestion, the derived graph (Loca), MCP, real-time sync. The previous Go implementation is archived in **`server-go-old/`** (deprecated reference for the port; the Rust modules mirror its layout — see `server-go-old/README.md`). Build: `cargo build --features loca`. The port is structural with honest in-code TODOs (streaming deltas, Discord port, persistent SPARQL query bridge) marked against `server-go-old/`.
...
- **Stack-history note**: `docs/why-go.md` argues against the old Electron+Python stack (lessons still hold). Go itself has now been replaced: the server is **Rust**, the Go code is archived in `server-go-old/`. See also `docs/versions-comparison.md`.
