...
impl Vault {
    ...
    /// All dates that have a `wiki/calendar/YYYY-MM-DD.md` page, as
    /// sorted `YYYY-MM-DD` id strings (R21-1). Read straight from disk
    /// — the date pages are canonical (generated by
    /// `generate_calendar_pages`); this just enumerates them so the
    /// desktop Calendar grid can mark which days have a page.
    pub fn calendar_date_ids(&self) -> Vec<String> {
        let mut out: Vec<String> = Vec::new();
        if let Ok(rd) = fs::read_dir(self.calendar_dir()) {
            for e in rd.flatten() {
                let p = e.path();
                if p.extension().and_then(|x| x.to_str()) != Some("md") {
                    continue;
    ...
    /// Supported kinds: person, note, event, project, task, calendar.
    pub fn get_entity(&self, kind: &str, id: &str) -> Option<EntityPage> {
        match kind {
            ...
            "calendar" => {
                // R21-2: id is "YYYY-MM-DD"; the page lives at
                // wiki/calendar/<id>.md. Mirrors the `note` arm so the
                // existing detail/markdown render path is reused. The
                // machine event-section sentinels are HTML comments,
                // so they stay invisible once rendered.
                let date = NaiveDate::parse_from_str(id, "%Y-%m-%d").ok()?;
                let content = fs::read_to_string(self.calendar_page_path(date)).ok()?;
                let (yaml, body) = split(&content);
                let fm: JsonValue = if yaml.is_empty() {
                    serde_json::Value::Object(Default::default())
                } else {
